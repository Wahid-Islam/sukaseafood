...
class CookingIntentOut(BaseModel):
    ...
    `source` says how it was produced: `rules` (deterministic keyword parse),
    `llm` (OpenAI fallback, used only when the rules found nothing), or
    `structured` (the client sent fields directly, e.g. after removing a chip).
    """
...
class SmartSwapRequest(BaseModel):
    ...
    Send `query` for a free-text request. Any structured field sent alongside
    overrides what was parsed from the text; send structured fields alone (no
    query) to re-rank after the user edits chips.
    """
